...
public interface UsrCompanyRelationAttachMapper {
	...
	@Select("select usr.nickname,company.is_management,company.join_time from usr_user usr,usr_company_relation company where usr.user_id = company.user_id and company.company_id = #{companyId} limit #{limitStart},#{limitEnd}")
