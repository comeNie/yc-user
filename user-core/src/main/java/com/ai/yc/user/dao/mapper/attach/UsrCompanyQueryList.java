...
public class UsrCompanyQueryList {
	...
	public String queryUserCompany(UserCompanyPageInfoRequest pageInfoRequest){
		...
		stringBuffer.append("select usr.usersource,company.company_name,usr.nickname,usr.firstname,company.telephone,company.linkman,company.create_time,company.state from usr_user usr,usr_company company where usr.user_id = company.admin_user_id and ");
		/**
		 * 待审核企业
		 */
	...
	public String queryUserCompanyCount(UserCompanyPageInfoRequest pageInfoRequest){
		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append("select count(*) a from usr_user usr,usr_company company where usr.user_id = company.admin_user_id and ");
		/**
		 * 待审核企业
		 */
		if(pageInfoRequest.getState()!=null){
