...
public interface UcGroupKeyInfoAttrAttachMapper {
    ...
    @Select("select groupInfo.*,userInfo.user_state,userInfo.user_login_name,userInfo.user_mp,userInfo.user_email,userInfo.email_validate_flag " +
    		"from uc_group_key_info groupInfo,uc_user userInfo where groupInfo.user_id = userInfo.user_id and groupInfo.user_id=#{userId} and groupInfo.audit_state=#{auditState} and groupInfo.tenant_id=#{tenantId}")
